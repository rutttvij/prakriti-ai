...
from app.models.bulk import BulkGenerator, OrganizationStatus
...
def activate_user(
    ...
    if user.role in (UserRole.BULK_MANAGER, UserRole.BULK_STAFF, UserRole.BULK_GENERATOR):
        ...
        if bulk_profile and bulk_profile.status != OrganizationStatus.APPROVED.value:
            raise HTTPException(
                status_code=409,
                detail="Approve organization first from Approvals.",
            )
